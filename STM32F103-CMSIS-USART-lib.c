...
//    Tested from 300 to 460,800 baud under 8 MHz CPU.
//
//    Mike Shegedin (EZdenki.com)
//
//      Version 1.1    9 Aug 2023   Added baudrate parameter to init function
//      Version 1.0   17 Jul 2023   Updated comments and core library
//      Started          Jul 2023
//
...
//  ------------------------------------------------------------------------------------------
//  Initalize USART port and set global USART pointer valriable and baud rate for this port
//    void
//    USART_init( USART_TypeDef *thisUSART, uint32_t USARTSpeed )
//  ------------------------------------------------------------------------------------------
...
USART_init( USART_TypeDef *thisUSART, uint32_t USARTSpeed )
{
  ...
  speedMant  = 8E6 / USARTSpeed / 16;
  speedFrac = ( 8E6 - USARTSpeed * speedMant * 16 ) / USARTSpeed;
  ...
  if( thisUSART == USART1 )
  {
    ...
    // Set Baudrate DIV_Mantissa and DIV_Fraction values to this 8 MHz CPU clock:
    thisUSART->BRR = ( speedMant << USART_BRR_DIV_Mantissa_Pos ) |
    ...
    // CNF3=01 (Floating Input), MODE3=00 (Input), CNF2=10 (Alt push-pull),
    // MODE2=00 (10 MHz).
  ...
  else  // Default to USART3
  {
    ...
    // CNF11=01 (Floating Input), MODE11=00 (Input), CNF10=10 (Alt push-pull),
    // MODE10=00 (10 MHz).
